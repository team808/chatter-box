...
module.exports = net.createServer(connectedClient => {
  ...
  connectedClient.on('data', data => {
    const parsedMessage = parseMessages(data.toString(), connectedClient.username);
    console.log(parsedMessage); // logs all messages when using @yesterday:
    if(parsedMessage.text) Message.create({ ...parsedMessage, nickname: connectedClient.username });
      ...
      case 'yesterday':
        chatRoom.getYesterday()
          .then(messages => {
            ...
            messages.forEach(message => connectedClient.write(`${message.nickname}: ${message.text}\n`));
            connectedClient.write('$$$');
          });
        break;
